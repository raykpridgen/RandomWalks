...
def read_shared_memory(shm_num, particleListFormat, partCount):
    # Setup shared memory (key should match C program)
    try:
        # Open the shared memory segment
        shm = sysv_ipc.SharedMemory(shm_num)
    ...
    unpacked = struct.unpack(particleListFormat, buffer)

    count = unpacked[partCount * 3]
    read_flag = unpacked[partCount * 3 + 1]
...
count = 0
while True:
    if read_shared_memory(SHM_KEY, particle_list_format, particle_count) == 1:
        count +=1
        if count == 3:
            break
